...
public class ProductDAO {
    ...
    public List<Product> getAllProducts() {
	  ...
	  public Product getProductById(int id) { String sql =
	  "SELECT * FROM products WHERE ProductID = ?"; try (Connection conn =
	  DBConnection.getConnection(); PreparedStatement stmt =
	  conn.prepareStatement(sql)) { stmt.setInt(1, id); ResultSet rs =
	  stmt.executeQuery();
	  
	  if (rs.next()) { Product p = new Product();
	  p.setProductID(rs.getInt("ProductID")); p.setName(rs.getString("Name"));
	  p.setDescription(rs.getString("Description"));
	  p.setPrice(rs.getDouble("Price")); p.setStock(rs.getInt("Stock"));
	  p.setDateAdded(rs.getDate("DateAdded"));
	  p.setCategoryID(rs.getInt("CategoryID")); p.setAdminID(rs.getInt("AdminID"));
 ...
 * 
 * if (rs.next()) { Product product = new Product();
 * product.setProductID(rs.getInt("productID"));
 * product.setName(rs.getString("name"));
 * product.setPrice(rs.getDouble("price")); // Set all other fields...
 * 
 * System.out.println("Found product: " + product.getName()); return product; }
 * else { System.out.println("No product found with ID: " + id); } } catch
 * (SQLException e) { System.err.println("SQL Error getting product by ID " +
 * id); System.err.println("SQL State: " + e.getSQLState());
 * System.err.println("Error Code: " + e.getErrorCode());
 * System.err.println("Message: " + e.getMessage()); e.printStackTrace(); }
 * return null; }
 */
